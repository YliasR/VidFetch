<script lang="ts">
  ...
  import { ipc, type MediaInfo, type GifDither } from '$lib/ipc';
  ...
  async function pickSource() {
    ...
    source = null;
    try {
      source = await ipc.probeMedia(picked);
      startText = '0:00';
  ...
  $: canExport =
    source != null && !exporting && !!outputPath && rangeError == null && fps >= 1 && fps <= 60;
...
<section class="view">
  <header class="title">
    <h2>Edit</h2>
    <p class="muted">
      Turn a downloaded video (or any local file) into a GIF. More editing tools land here in
      future nightlies.
    </p>
  </header>
